...
import 'package:untitled1/shared/components/screen_of_tasks_body_separated_list.component.dart';
...
class DoneTasks extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<TODOAppCubit , TODOStateBase>(builder: (context , state){
      ...
      listOfTasks = listOfTasks.where((element) => element.state == TODOAPPTASKStatuses.DONE).toList();
      if (listOfTasks.isEmpty) {
        return  const NoTasksAddYetComponent();
      }
      return ScreenOfTasksBodySeparatedList(listOfTasks: listOfTasks, cubit: cubit);
