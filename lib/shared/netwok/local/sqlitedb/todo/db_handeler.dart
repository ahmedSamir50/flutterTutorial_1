...
class ToDoAppDbAHandler {
  ...
  static final ToDoAppDbAHandler instance =
      ToDoAppDbAHandler._privateConstructor();
  ...
  FutureOr<void> OnCreateDb(Database db, int version) async {
    try {
      ...
      printOnyOnDebugMode(
          ["*****Error while Creating the Db***** : ", onError]);
    }
  }
  ...
  Future<int> UpdateTaskStatus(int taskID,
      {TODOAPPTASKStatuses taskNewState = TODOAPPTASKStatuses.DONE}) async {
    return (await instance.database).rawUpdate(
        ''' Update Tasks Set state = '$taskNewState' , LastStateTime = datetime('now') Where id = $taskID  ''');
  }

  Future<List<TODOModel>> GetAllTodos() async {
    var dbList = await (await instance.database)
        .rawQuery("SELECT * FROM Tasks order by date(timeOfCreation) DESC ");
    List<TODOModel> modelList =
        (dbList).map((e) => TODOModel.fromMap(e)).toList();
    return modelList;
  }
}
